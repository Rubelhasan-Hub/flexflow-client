        ...
        <div className="p-4 md:p-8 min-h-screen text-white">
                        ...
                        {loading ? (
                            ...
                            bookings.map((booking) => (
                                <tr key={booking._id} className="hover:bg-slate-800 transition">
                                    ...
                                    <td className="p-4">
                                        <span className="px-2 py-1 bg-green-900/50 text-green-400 border border-green-700 rounded text-xs uppercase">{booking.status}</span>
                                    </td>
                                    <td className="p-4">
            ...
            <div className="md:hidden space-y-4">
                {loading ? (
                    ...
                    bookings.map((booking) => (
                        <div key={booking._id} className="bg-[#1e293b] p-4 rounded-xl border border-slate-700 space-y-3">
                            <div className="flex justify-between items-start">
                                <h3 className="font-semibold text-blue-400 text-lg">{booking.className}</h3>
                                <span className="px-2 py-1 bg-green-900/50 text-green-400 border border-green-700 rounded text-[10px] uppercase">{booking.status}</span>
                            </div>
                            <p className="text-sm text-slate-300"><span className="text-slate-500">Trainer:</span> {booking.trainerName}</p>
                            <p className="text-sm text-slate-300"><span className="text-slate-500">Schedule:</span> {booking.scheduleDays?.join(", ")}</p>
                            <Link href={`/classes/${booking.classId}`} className="block">
                                <button className="w-full flex justify-center items-center gap-1.5 px-3 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-sm transition">
                    ...
                    <p className="text-center text-slate-500">You have not enrolled in any classes yet.</p>
                )}
            </div>
        </div>
    );
